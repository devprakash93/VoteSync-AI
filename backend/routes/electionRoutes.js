...
const { createElection, getElections, getElectionById, getCompletedElections, addCandidate, endElection, deleteElection, deleteCandidate } = require('../controllers/electionController');
const { protect, admin } = require('../middleware/authMiddleware');

router.route('/').post(protect, admin, createElection).get(protect, getElections);
router.route('/completed').get(getCompletedElections); // public — for results page
